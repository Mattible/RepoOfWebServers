...
class TestWebServerHandler(unittest.TestCase):
    ...
    def test_do_get_health_endpoint(self):
        ...
        with patch.object(self.handler, '_send_health_response') as mock_health:
            self.handler.do_GET()
    ...
    def test_send_hello_response(self):
        ...
        with patch.object(self.handler, '_send_response') as mock_send:
            ...
            mock_send.assert_called_once_with(200, "Hello World!\n", 'text/plain')
        ...
        with patch.object(self.handler, 'send_response') as mock_status:
            with patch.object(self.handler, 'send_header') as mock_header:
                with patch.object(self.handler, 'end_headers') as mock_end:
                    with patch.object(self.handler.wfile, 'write') as mock_write:
                        
                        self.handler._send_response(200, "Test content", 'text/plain')
                        
                        mock_status.assert_called_once_with(200)
                        mock_header.assert_any_call('Content-Type', 'text/plain')
                        mock_header.assert_any_call('Content-Length', '12')  # len("Test content")
...
class TestMainFunction(unittest.TestCase):
    ...
    def test_main_function_custom_host(self):
        ...
        with patch.dict(os.environ, {'HOST': '192.168.1.100', 'WEBSERVER_PORT': '8080'}):
            with patch('server.PythonWebServer') as mock_server_class:
                mock_server = MagicMock()
                mock_server_class.return_value = mock_server
                ...
                mock_server_class.assert_called_once_with('192.168.1.100', 8080)
...
class TestIntegration(unittest.TestCase):
    ...
    def test_error_handling_integration(self):
        ...
        handler = self._create_mock_handler('/nonexistent/path/with/params?test=123')
        ...
        with patch.object(handler, '_send_json_response') as mock_json:
            ...
            error_data = args[1]
            self.assertEqual(error_data['status_code'], 404)
            self.assertIn('/nonexistent/path/with/params?test=123', error_data['message'])
