use rust_webserver::{ServerInfo, WebServer};
...
fn test_all_endpoints_responses() {
    ...
    // Test all endpoints
    let endpoints = vec![("/", "Hello, world!"), ("/health", "OK"), ("/image", "")];
...
fn test_request_parsing_edge_cases() {
    ...
    assert_eq!(WebServer::parse_http_request("GET /"), None);
    assert_eq!(
        WebServer::parse_http_request("GET / HTTP/1.1"),
        Some(("GET", "/"))
    );
    assert_eq!(
        WebServer::parse_http_request("POST /api HTTP/1.1"),
        Some(("POST", "/api"))
    );
}
